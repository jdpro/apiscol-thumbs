package fr.ac_versailles.crdp.apiscol.thumbs.filters;
...
public class SecurityFilter implements ContainerRequestFilter {
	...
	public ContainerRequest filter(ContainerRequest request) {
		...
		if (method.equals(HttpMethod.PUT) || method.equals(HttpMethod.POST)
			...
			String secret = ThumbsApi.getProperty(
					ParametersKeys.sharedSecret, context);
			...
			String computedHashedEtag = SecurityUtils.hashWithSharedSecret(
					etag, secret);
			if (!StringUtils.equals(providedHashedEtag, computedHashedEtag))
				throw new WebApplicationException(
						Response.status(401)
								.entity("Invalid authorization token in thumbs web service")
								.build());
		}
		return request;
	}
